...
if __name__ ==  '__main__':
    ...
    try:
        ...
        time_term = job_term[2].split()[2][:-1].split(':')
        hours = int(time_term[0])
        minutes = int(time_term[1])
        seconds = int(time_term[2])
        ...
        flocking_line = [i for i, x in enumerate(subcontent) if '+WantFlocking' in x][0]
        glidein_line = [i for i, x in enumerate(subcontent) if '+WantGlidein' in x][0]

        if hours > 6:
            subcontent[flocking_line] = '\n'
            subcontent[glidein_line] = '\n'
