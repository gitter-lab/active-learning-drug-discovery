"""
    Runs an active learning simulation for experiment 3.
    Experiment 3: 1 random initial start for 50 iters ON ~107 target/task datasets.
    
    Usage:
        python experiment_3_runner.py \
        --pipeline_params_json_file=../param_configs/exp_3_pipeline_config.json \
        --hyperparams_json_file=../param_configs/experiment_3_hyperparams/benchmarks/ClusterBasedRandom.json \
        --initial_dataset_file=../datasets/pcba/pcba-aid995_cv_96/unlabeled_13.csv
        --task_name=pcba-aid995 \
        --iter_max=50 \ 
        --batch_size_index=0 \
...
if __name__ ==  '__main__':
    ...
    # update unlabeled data path and task_name
    pipeline_config["unlabeled_data_params"]["data_path_format"] = pipeline_config["unlabeled_data_params"]["data_path_format"].format(task_name, '{}')
    pipeline_config["common"]["task_names"] = pipeline_config["common"]["task_names"].format(task_name)
