...
if __name__ ==  '__main__':
    ...
    parser.add_argument('--params_json_file', action="store", dest="params_json_file", required=True)
    ...
    params_json_file = given_args.params_json_file
    cycle_num = given_args.cycle_num
    process_num = given_args.process_num
    
    # load param json config
    with open(params_json_file) as f:
        config = json.load(f)
        
    # load training data
    training_loader = prepare_loader(data_loader_params=config["training_data_params"], 
    ...
    model = load_model(model_type=config["model"]["type"]
    ...
    # load unlabeled pool
    unlabeled_loader = prepare_loader(data_loader_params=config["unlabeled_data_params"], 
                                      task_names=config["common"]["task_names"],
                                      smile_col_name=config["common"]["smile_col_name"],
                                      feature_name=config["common"]["feature_name"],
                                      cluster_col_name=config["common"]["cluster_col_name"],,
